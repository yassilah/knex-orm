...
export type TableItem<S extends Schema, T extends TableNames<S>, Deep = true> = Prettify<{
   [K in TableColumnNames<S, T, Deep extends true ? false : true>]: S[T][K] extends infer U
      ? U extends BelongsToRelationDefinition
         ...
         : U extends ColumnDefinition ? InferColumnType<U>
            : never
      : never
...
export type TablePrimaryKeyValue<S extends Schema, N extends TableNames<S>> = TablePrimaryKeyName<S, N> extends infer PK extends TableColumnNames<S, N>
